...
module.exports = {
  ...
  async execute(interaction) {
      ...
      const embed = new EmbedBuilder()
       ...
       if (page === 'basic') {
        ...
        const displayFlags = userFlags.length > 0
            ? userFlags.map(flag => `${flagEmojis[flag] || '❓'} \`${flag.replace(/([A-Z])/g, ' $1').trim()}\``).join('\n')
            : '✨ None';
    ...
    const clientStatus = presence.clientStatus
      ...
      const activities = presence.activities || []; // Ensure this is defined here
...
const customActivity = activities.find(a => a.type === 4);
    ...
    if (customActivity) {
        ...
        if (customActivity.emoji) {
            if (customActivity.emoji.id) { // Custom emoji
        ...
        console.log('No Custom Activity Found in activities array.'); 
    }
    ...
    embed.setTitle(`📊 Activity & Status for ${member.user.tag}`)
         .addFields(
            ...
            { name: 'Custom Status', value: customStatusText, inline: false }, // This will now correctly display the custom status
